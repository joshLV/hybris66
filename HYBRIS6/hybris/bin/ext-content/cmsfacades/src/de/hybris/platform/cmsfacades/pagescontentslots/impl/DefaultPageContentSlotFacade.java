...
public class DefaultPageContentSlotFacade implements PageContentSlotFacade
{
	...
	/**
	 * {@inheritDoc}
	 * @deprecated since version 6.3; Please use {@link #getContentSlotsByPage(String)} to retrieve shared content slot
	 *             information instead.
	 */
	@Deprecated
	@HybrisDeprecation(sinceVersion = "6.3")
	@Override
	public List<PageContentSlotData> getPagesBySlotId(final String slotId) throws CMSItemNotFoundException, ConversionException
	{
		ContentSlotModel contentSlot = null;
		try
		{
			contentSlot = getAdminContentSlotService().getContentSlotForId(slotId);
		}
		catch (UnknownIdentifierException | AmbiguousIdentifierException e)
		{
			throw new CMSItemNotFoundException("Can not find content slot with uid \"" + slotId + "\".", e);
		}

		return Stream
				// Find all page relations where the content slot is defined
				.concat(getAdminContentSlotService().getPageRelationsForSlot(contentSlot).stream(),
						// Find all template relations where the content slot is defined and not overridden or where the template is not used in any page
						getAdminContentSlotService().getTemplateRelationsForSlot(contentSlot).stream())
				.map(relation -> buildPageContentSlot(relation)).collect(Collectors.toList());
	}
	...
	/**
	 * Build a single <code>PageContentSlotData<code> by calling the converter for the corresponding type of the CMS
	 * Relation model
	 *
	 * @param relation
	 *           - the CMS Relation model
	 * @return a new DTO
	 * @throws ConversionException when a <code>PageContentSlotConverterType</code> is not defined for the specified
	 *            relation class name.
	 * @deprecated since version 6.3
	 */
	@Deprecated
	@HybrisDeprecation(sinceVersion = "6.3")
	protected PageContentSlotData buildPageContentSlot(final CMSRelationModel relation) throws ConversionException
	{
		final PageContentSlotConverterType converterType = getPageContentSlotConverterRegistry()
				.getPageContentSlotConverterType(relation.getClass()).orElseThrow(() -> new ConversionException(
						String.format("Converter not found for CMS Relation Model [%s]", relation.getClass().getName())));
		return converterType.getConverter().convert(relation);
	}
