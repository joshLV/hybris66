...
describe('smartEditContractChangeListener', function() {
    ...
    beforeEach(module('componentHandlerServiceModule', function($provide) {
        ...
        resizeListener = jasmine.createSpyObj('resizeListener', ['register', 'unregister', 'fix', 'dispose']);
    ...
    describe('DOM intersections', function() {
        ...
        it('when components are no longer visible, they are destroyed', function() {
            ...
            expect(resizeListener.unregister.calls.count()).toBe(2);
            expect(resizeListener.unregister.calls.argsFor(0)[0]).toBe(parent);
            expect(resizeListener.unregister.calls.argsFor(1)[0]).toBe(component1);

            expect(resizeListener.fix.calls.count()).toBe(1);
            expect(resizeListener.fix.calls.argsFor(0)[0]).toBe(parent);

            expect(positionRegistry.register).not.toHaveBeenCalled();
            expect(positionRegistry.unregister.calls.count()).toBe(2);
            expect(positionRegistry.unregister.calls.argsFor(0)[0]).toBe(parent);
            expect(positionRegistry.unregister.calls.argsFor(1)[0]).toBe(component1);
            expect(onComponentsAddedCallback).not.toHaveBeenCalled();
    ...
    describe('DOM mutations', function() {
        ...
        it('should be able to observe sub tree of smartEditComponent (and parent) removed', function() {
            ...
            // THEN
            expect(resizeListener.unregister.calls.count()).toEqual(3);
            expect(resizeListener.unregister.calls.argsFor(0)[0]).toBe(component2);
            expect(resizeListener.unregister.calls.argsFor(1)[0]).toBe(component2_1);
            expect(resizeListener.unregister.calls.argsFor(2)[0]).toBe(component3);

            expect(resizeListener.fix.calls.count()).toBe(1);
            expect(resizeListener.fix.calls.argsFor(0)[0]).toBe(parent);

            expect(positionRegistry.unregister.calls.count()).toEqual(3);
            expect(positionRegistry.unregister.calls.argsFor(0)[0]).toBe(component2);
            expect(positionRegistry.unregister.calls.argsFor(1)[0]).toBe(component2_1);
            expect(positionRegistry.unregister.calls.argsFor(2)[0]).toBe(component3);
