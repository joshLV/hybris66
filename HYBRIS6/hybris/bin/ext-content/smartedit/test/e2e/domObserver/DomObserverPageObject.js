...
module.exports = (function() {
    ...
    page.assertions = {
        ...
        overlayAndStoreFrontAreSynced: function() {
            ...
            browser.waitUntil(function() {
                return protractor.promise.all([page.elements.getTotalVisibleStoreFrontElements(), page.elements.getTotalRepositionListeners()]).then(function(array) {
                    return Number(array[0]) === Number(array[1]);
                });
            }, "Number of reposition listeners different from the number of visible storefront elements");

            browser.waitUntil(function() {
                return protractor.promise.all([page.elements.getTotalVisibleStoreFrontElements(), page.elements.getTotalResizeListeners()]).then(function(array) {
                    return Number(array[0]) === Number(array[1]);
                });
