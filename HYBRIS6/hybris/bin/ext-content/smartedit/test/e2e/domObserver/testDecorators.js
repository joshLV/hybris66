...
angular.module('FakeModule', [
    ...
    .controller('sakExecutorDebugController', function($interval, sakExecutor, yjQuery, COMPONENT_CLASS, ID_ATTRIBUTE, resizeListener, positionRegistry, smartEditContractChangeListener, CONTRACT_CHANGE_LISTENER_INTERSECTION_OBSERVER_OPTIONS) {
        ...
        $interval(function() {
            this.inPageElements = Array.prototype.slice.apply(yjQuery("." + COMPONENT_CLASS));
            this.inPageElements.forEach(function(element) {
                // intersectionObserver.unobserve(element); //FIXME: bug in Firefox on toggle perspective empty <--> all
        ...
        this.$onInit = function() {
            ...
            this.getSakExecutorElements = function() {
                return sakExecutor.getScopes().map(function(scope) {
    ...
    .component('sakExecutorDebug', {
        template: '<button class="btn btn-info btn-sm toggleDebug" ng-click="ctrl.toggleDebug();"><span ng-if="!ctrl.showDebug">open</span><span ng-if="ctrl.showDebug">close</span> debug</button>' +
            ...
            '<pre><span>{{ctrl.assertAllGood() ? "OK" : "ERROR (possible leak)"}}</span></pre>' +
            '</div>',
