...
angular.module("smartEditContractChangeListenerModule", [
    ...
    .service("smartEditContractChangeListener", function(
         ...
         */
        var aggregateAddedNodesAndTheirParents = function(mutations) {
            var entries = lodash.flatten(mutations.filter(function(mutation) {
                //only keep mutations of type childList and addedNodes
                return mutation.type === MUTATION_TYPES.CHILD_LIST.NAME && mutation[MUTATION_TYPES.CHILD_LIST.ADD_OPERATION] && mutation[MUTATION_TYPES.CHILD_LIST.ADD_OPERATION].length;
            }).map(function(mutation) {

                //the mutated child may not be smartEditComponent, in such case we return their first level smartEditComponent children
                var children = lodash.flatten(Array.prototype.slice.call(mutation[MUTATION_TYPES.CHILD_LIST.ADD_OPERATION])
        ...
        var repairParentResizeListener = function(parent) {
            if (parent) {
                ...
                if (parentObj && parentObj.processed === COMPONENT_STATE.ADDED && yjQuery.contains($document[0], parent)) {
        ...
        var mutationObserverCallback = function(mutations) {
            ...
            if (this._componentsAddedCallback) {
                aggregateAddedNodesAndTheirParents(mutations).forEach(function(childAndParent) {
            ...
            if (!mutationObserver) {
                ...
                if (!intersectionObserver) {
                    ...
                    intersectionObserver = this._newIntersectionObserver(function(entries) {
                        ...
                        // The RESTART_PROCESS is used here to process the componentsQueue.
        ...
        this._rawProcessQueue = function() {
            systemEventService.sendSynchEvent(CONTRACT_CHANGE_LISTENER_PROCESS_EVENTS.PROCESS_COMPONENTS, lodash.map(this.componentsQueue, 'component')).then(function(components) {
                var addedComponents = [];
                var removedComponents = [];
                this.componentsQueue.forEach(function(obj) {
                    var componentProcessStatus = components.find(function(component) {
                        return component === obj.component;
                    }).dataset[SMARTEDIT_COMPONENT_PROCESS_STATUS];
                    if (componentProcessStatus === CONTRACT_CHANGE_LISTENER_COMPONENT_PROCESS_STATUS.PROCESS) {
                        if (obj.isIntersecting && obj.processed !== COMPONENT_STATE.ADDED) {
                            addedComponents.push(obj);
                        } else if (!obj.isIntersecting && obj.processed === COMPONENT_STATE.ADDED) {
                            removedComponents.push(obj);
                        }
                    } else if (componentProcessStatus === CONTRACT_CHANGE_LISTENER_COMPONENT_PROCESS_STATUS.REMOVE) {
                ...
                lodash.chain(addedComponents.concat(removedComponents)).map('parent').compact().uniq().value().forEach(function(parent) {
                    repairParentResizeListener(parent);
                });
            }.bind(this)).finally(this._clearQueue.bind(this));
        };

        this._throttledProcessQueue = lodash.throttle(this._rawProcessQueue, CONTRACT_CHANGE_LISTENER_PROCESS_QUEUE_THROTTLE);

        // when a component in queue have isIntersecting:false (i.e not in the viewport) and is removed from DOM, it will not be included in the Intersection Observer callback entries.
        // the queue will not process these components so we must clear it to remove the components that are not anymore in the DOM.
        this._clearQueue = function() {
            this.componentsQueue.filter(function(queueObj) {
                return !yjQuery.contains($document[0], queueObj.component);
        ...
        this._addComponents = function(componentsObj) {
            if (this._componentsAddedCallback) {
                executeCallback(this._componentsAddedCallback.bind(undefined, lodash.map(componentsObj, 'component')));
            }
            componentsObj.forEach(function(queueObj) {
                if (this._componentRepositionedCallback) {
                    positionRegistry.register(queueObj.component);
                }
                if (this._componentResizedCallback) {
                    resizeListener.register(queueObj.component, this._componentResizedCallback.bind(undefined, queueObj.component));
                }
            }.bind(this));
        };
        ...
        this._removeComponents = function(componentsObj) {
            componentsObj.forEach(function(queueObj) {
                this._unregisterSizeAndPositionListeners(queueObj);

                if (!yjQuery.contains($document[0], queueObj.component)) {
                    this.componentsQueue.splice(this._getComponentIndexInQueue(queueObj.component), 1);
                }
            }.bind(this));
            if (this._componentsRemovedCallback) {
                ...
                executeCallback(this._componentsRemovedCallback.bind(undefined, removedComponents));
            }
        };

        this._unregisterSizeAndPositionListeners = function(queueObj) {
            if (this._componentRepositionedCallback) {
                positionRegistry.unregister(queueObj.component);
            }
            if (this._componentResizedCallback) {
                resizeListener.unregister(queueObj.component);
            }
        };
