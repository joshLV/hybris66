...
let sfBuilder = function() {
    ...
    function StorefrontBuilder() {
        ...
        this.rerender = function(componentId, componentType, parentComponentIds) {
            ...
            if (!componentId) {
                ...
                let idChain = parentComponentIds ? parentComponentIds + ',' + componentId : componentId;
                treeUtils.treeForEach(this.config.tree, function(node, index) {
                    let strChain = node.idChainFromRootNode.toString();
                    if (strChain.endsWith(idChain)) {
                        let element = createSelectorFromIdChain(node.idChainFromRootNode);
                        $(element).remove();
                        addNode(node, index);
                    }
                });
        ...
        this.removeComponent = function(componentId, parentComponentIds) {
            ...
            function breadthFirstRemoval(node) {
                let str = node.idChainFromRootNode.toString();
                if (str.endsWith(idChain)) {
                    ...
                    node.children.forEach(function(child, index) {
                        if (breadthFirstRemoval(child)) {
                            ...
                            let selectorForChildNode = createSelectorFromIdChain(child.idChainFromRootNode);
        ...
        this.addComponent = function(presetAlias, parentComponentIds) {
            let libNode = sfNodeLib.getNode(presetAlias);

            if (!parentComponentIds) {
                let root = this.config.tree;
                let newIdChain = testLodash.cloneDeep(root.idChainFromRootNode);
                let newNode = testLodash.cloneDeep(libNode);
                ...
                treeUtils.treeForEach(this.config.tree, function(node) {
                    ...
                    if (strChain.endsWith(parentComponentIds)) {
                        ...
                        let newIdChain = testLodash.cloneDeep(node.idChainFromRootNode);
                        let newNode = testLodash.cloneDeep(libNode);
                        newIdChain.push(newNode.componentId);
                        newNode.idChainFromRootNode = newIdChain;
        ...
        this.removeNodesFromTree = function(componentId, parentComponentIds) {
            let selector = parentComponentIds ? (parentComponentIds + ',' + componentId) : componentId;
            treeUtils.treeForEach(this.config.tree, function f(node) {
                let indicesToRemove = [];
                node.children.forEach(function(child, index) {
                    let strChain = child.idChainFromRootNode.toString();
        ...
        this.addNodeToTree = function(alias, parentComponentIds, position) {
            position = position || 0;
            let libNode = sfNodeLib.getNode(alias);
            if (!parentComponentIds) {
                let root = this.config.tree;
                let newIdChain = testLodash.cloneDeep(root.idChainFromRootNode);
                let newNode = testLodash.cloneDeep(libNode);
        ...
        this.moveComponent = function(componentId, parentComponentIds, newPosition) {
            treeUtils.treeForEachParent(this.config.tree, parentComponentIds, function(parentNode) {
                ...
                let parentElement = $(createSelectorFromIdChain(parentNode.idChainFromRootNode));
                ...
                let elementToMove = parentElement.children(domUtils.SELECTORS.COMPONENT_AT_INDEX(oldPosition));
                ...
                if (newPosition === 0) {
                    ...
                    let c = parentElement.children(domUtils.SELECTORS.ANY_COMPONENT_INDEX);
                    let onTop = $(c.get(newPosition - 1));
        ...
        this.transformComponent = function(componentId, parentComponentIds, presetAlias) {
            ...
            treeUtils.treeForEach(this.config.tree, function(node, index) {
                let strChain = node.idChainFromRootNode.toString();
                if (strChain.endsWith(selector)) {
                    let existingElement = $(createSelectorFromIdChain(node.idChainFromRootNode));
        ...
        let componentLoadedCallbackRegistry = {};
        ...
        function createParentSelectorFromNode(node) {
            let ids = node.idChainFromRootNode || [];
        ...
        function applyStorefrontRenderStrategy(conf) {
            let namespace = window.smartedit || {};
            namespace.renderComponent = conf.renderer;
        }


        function insertElementAtNodeIndex(node, element, index) {
            let parentElement = createParentSelectorFromNode(node);
            if (parentElement) {
                parentElement = $(parentElement);
            } else {
                parentElement = domUtils.getContentRoot();
            }
            let siblings = parentElement.children(domUtils.SELECTORS.ANY_COMPONENT_INDEX);
            if (siblings.length === 0) {
                parentElement.append(element);
                return;
            }
            for (let i = 0; i < siblings.length; i++) {
                let jqSibling = $(siblings[i]);
                let currentIndex = Number(jqSibling.attr(domUtils.ATTRIBUTES.UNIQUE_INDEX));
        ...
        function addNode(node, index) {
            ...
            let componentLoadDelay = delayUtils.getComponentLoadDelay(node.componentId);
            let addNodeFn = addElementToDOMAndProcessNodeChildren.bind(this, newElement, node, index);
        ...
        function getTemplateUrl(node) {
            let defaultTemplatePath = './layouts/';
            let base;
            ...
            let templateUrl = base + node.nodeType + '/' + node.componentId + '.html';
        ...
        function updateElement(node, element) {
            ...
            let loadContentFunction = function() {
                element.append(contentRoot);
            };
            let contentLoadDelay = delayUtils.getContentLoadDelay(node.componentId);
    ...
    function DOMUtils() {
        ...
        this.updatePageIdAttribute = function(pageId, catalogVersion) {
            let catalogClassName = this.ATTRIBUTES.PAGE_CATALOG_VERSION_PREFIX + catalogVersion;
            let uid = this.ATTRIBUTES.PAGE_UID_PREFIX + pageId;
            let uuid = this.ATTRIBUTES.PAGE_UUID_PREFIX + pageId;
            let rejectedClassPrefixes = [
                this.ATTRIBUTES.PAGE_UID_PREFIX,
                this.ATTRIBUTES.PAGE_UUID_PREFIX,
                catalogClassName
            ];
            let currentClasses = ($(this.SELECTORS.BODY).attr('class') || "").split(' ');
            ...
            let classes = uid + ' ' + uuid + ' ' + catalogClassName + ' ' + currentClasses.join(' ');
            $(this.SELECTORS.BODY).attr('class', classes);

            let piip = $(this.SELECTORS.PAGE_ID_INSERTION_POINT);
            if (piip.length > 0) {
                piip.html(pageId);
            }
        };
