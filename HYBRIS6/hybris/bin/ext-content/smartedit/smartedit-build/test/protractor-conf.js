...
exports.config = {
    ...
    onPrepare: function() {
        ...
        // =============================================
        global.e2e = {};
        let globResult = glob.sync('**/testDependencyLoader.js'); // some extensions don't have a testDependency loader
        if (globResult.length === 1 && fs.existsSync(path.resolve(globResult[0]))) {
            try {
                let dependencyLoader = require(path.resolve(globResult[0]));
                dependencyLoader.loadE2eDependencies(browser, global.e2e);
            } catch (e) {
                console.error(`protractor-conf.onPrepare(): Failure loading testDependencyLoader ${path.resolve(globResult[0])}`);
                console.error(e);
            }
        } else {
            console.info('No testDependencyLoader found.');
        }
        ...
        beforeEach(function() {
            ...
            if (browser.setStorefrontDelayConfigInSessionStorage) {
                browser.setStorefrontDelayConfigInSessionStorage();
            }

            browser.driver.manage().timeouts().implicitlyWait(0).then(function() {
