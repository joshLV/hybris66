...
public class MediaController extends AbstractSearchableController
{
	...
	@ApiResponses({
			@ApiResponse(code = 400, message = "When no media is found matching the given uuid (WebserviceValidationException)."),
			@ApiResponse(code = 200, message = "Media data", response = MediaData.class)
	})
	public MediaData getMediaByUuid(
			@ApiParam(value = "The universally unique identifier of the media item", required = true) @PathVariable final String uuid)
	...
	public MediaListData getMediaByQuery(
			@ApiParam(value = "The name of the named query to use for the search.", required = true) @ModelAttribute("namedQuery") final NamedQueryData namedQuery)
			throws WebserviceValidationException
	{
