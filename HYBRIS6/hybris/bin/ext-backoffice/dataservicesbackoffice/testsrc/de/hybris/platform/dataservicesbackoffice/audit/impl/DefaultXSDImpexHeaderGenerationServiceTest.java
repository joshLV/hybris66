...
{
	private static final Logger log = LoggerFactory.getLogger(DefaultXSDImpexHeaderGenerationServiceTest.class);
	...
	public void testGenerateXSDReportAgainstImpex() throws Exception
	{
		...
		//User
		assertEquals(userPersisted.getName(), "mockUser");
		assertEquals(userPersisted.getUid(), "mockUUID");

		//Currency
		assertEquals(userPersisted.getSessionCurrency().getIsocode(), "EUR");

		//Media
		assertEquals(userPersisted.getProfilePicture().getCode(), "mockMedia");

		//UserGroup
		assertEquals(userPersisted.getGroups().iterator().next().getUid(), "orgadmingroup");

		//Address
		assertEquals(userPersisted.getDefaultPaymentAddress().getStreetname(), "mockStreet");
		assertEquals(userPersisted.getDefaultPaymentAddress().getTown(), "mockTown");

		//Title
		assertEquals(userPersisted.getDefaultPaymentAddress().getTitle().getName(), "Mock");
