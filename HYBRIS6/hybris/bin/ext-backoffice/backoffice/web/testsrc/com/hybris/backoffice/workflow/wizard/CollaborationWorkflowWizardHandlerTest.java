...
public class CollaborationWorkflowWizardHandlerTest
{
	...
	public void setUp()
	{
		...
		doNothing().when(handler).notifyUser(same(createdWorkflow), anyString(), any());
	}
	...
	public void testWorkflowCreatedAndStartedSuccess()
	{
		...
		verify(handler).notifyUser(createdWorkflow, WorkflowConstants.EVENT_TYPE_WORKFLOW_CREATED_AND_STARTED,
	...
	public void testAdHocWorkflowWorkflowCreated()
	{
		...
		verify(handler).notifyUser(createdWorkflow, WorkflowConstants.EVENT_TYPE_WORKFLOW_CREATED, NotificationEvent.Level.SUCCESS);
		verify(workflowEventPublisher).publishWorkflowUpdatedEvent(createdWorkflow);
	}
