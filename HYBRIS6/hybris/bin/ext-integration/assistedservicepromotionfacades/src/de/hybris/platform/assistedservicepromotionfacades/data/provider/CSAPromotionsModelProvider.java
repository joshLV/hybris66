...
public class CSAPromotionsModelProvider implements FragmentModelProvider<List<CSAPromoData>>
{
	...
	public List<CSAPromoData> getModel(final Map<String, String> parameters)
	{
		...
		if(CollectionUtils.isNotEmpty(csaAllPromos) && (CollectionUtils.isNotEmpty(firedPromos) || CollectionUtils.isNotEmpty(potentialPromos)))
		{	
			//Populating Fired promos
			populateFiredOrPotentialPromosFromCart(Boolean.TRUE,csaAllPromos,csaPotentialAndFiredPromos,
					firedPromos.stream().filter(firedPromo -> firedPromo.getPromotionData().getCode().startsWith(searchKeyword)).collect(Collectors.toList()));
			
			//Populating Potential promos
			populateFiredOrPotentialPromosFromCart(Boolean.FALSE,csaAllPromos,csaPotentialAndFiredPromos,
					potentialPromos.stream().filter(potentialPromo -> potentialPromo.getPromotionData().getCode().startsWith(searchKeyword)).collect(Collectors.toList()));
			
			Collections.sort(csaPotentialAndFiredPromos, new Comparator<CSAPromoData>(){
	        @Override
	        public int compare(CSAPromoData promo1, CSAPromoData promo2)
	        {
	...
	protected void populateFiredOrPotentialPromosFromCart(Boolean fired,List<CSAPromoData> csaAllPromos, List<CSAPromoData> csaPotentialAndFiredPromos, List<PromotionResultData> cartPromos){
		...
		cartPromos.stream().forEach(firedPromo ->
		{
			...
			CSAPromoData cartPromo =  csaAllPromos.stream().filter(csaPromo -> csaPromo.getCode().equalsIgnoreCase(firedPromo.getPromotionData().getCode())).findFirst().get();
