		...
		if (actionResult.getResult() == ActionResult.OK && item instanceof SAPRFCDestination)
		{
			try
			{
				if ((currentValues != null && !currentValues.isEmpty())
						&& (item.getAttribute(SAPRFCDestinationEvent.RFC_DESTINATION_NAME) != null))
				{
					// don't publish event in case of turn on / off JCo trace
					// and / or set JCo trace path
					if ((currentValues.size() > 2)
							|| ((currentValues.size() <= 2 && !currentValues.containsKey(SAPRFCDestinationEvent.JCO_TRACE_LEVEL)
									&& !currentValues.containsKey(SAPRFCDestinationEvent.JCO_TRACE_PATH))))
					{
						getEventService().publishEvent(new SAPRFCDestinationUpdateEvent(
								item.getAttribute(SAPRFCDestinationEvent.RFC_DESTINATION_NAME).toString()));
					}
					else
					{
						...
						{
							final SAPRFCDestinationJCoTraceEvent event = new SAPRFCDestinationJCoTraceEvent(
									currentValues.get(SAPRFCDestinationEvent.JCO_TRACE_LEVEL).toString());
