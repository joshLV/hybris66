...
public class UiStateHandler
{
	...
	/**
	 * Get the id of the first group containing a cstic.<br>
	 * A UI group may be empty, or only containing other UI group, but no cstics.
	 *
	 * @param configData
	 *           actual configuration
	 * @return id of the first group containing a cstic
	 */
	...
	public void compileGroupForDisplay(final ConfigurationData configData, final UiStatus uiStatus)
	{
		...
		if (configData.getCpqAction() != null
				&& (configData.getCpqAction().equals(CPQActionType.NAV_TO_CSTIC_IN_CONFLICT) || configData.getCpqAction().equals(
						CPQActionType.NAV_TO_CSTIC_IN_GROUP)))
		{
			...
			final Deque<String> path, final Deque<String> groupIdPath, final String groupsIdentifier)
	{
		...
		if (groups != null)
		{
			for (int i = 0; i < groups.size(); i++)
			{
				...
				final UiGroupData matchingSubGroup = compileGroupForDisplay(group.getSubGroups(), groupIdToDisplay, path,
						groupIdPath, PATHELEMENT_SUBGROUPS);
