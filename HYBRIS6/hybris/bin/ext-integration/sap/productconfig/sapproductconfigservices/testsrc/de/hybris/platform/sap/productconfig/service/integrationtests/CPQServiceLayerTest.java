...
public abstract class CPQServiceLayerTest extends ServicelayerTest
{
	...
	private static final String CATALOG_ID = "testConfigureCatalog";
	...
	protected void prepareCPQData() throws Exception
	{
		...
		final SAPConfigurationModel sapConfigModel = getFromPersistence("Select {pk} from {sapconfiguration}");
		final ClassificationSystemModel classificationSystemModel = getFromPersistence("select {pk} from {classificationSystem} where {id}='cpqTestClassification'");
		baseSite = getFromPersistence("Select {pk} from {CMSSite} where {uid}='testConfigureSite'");
		baseStore = getFromPersistence("Select {pk} from {BaseStore} where {uid}='testConfigureStore'");
		...
		baseStoreList.add(baseStore);
		baseSite.setStores(baseStoreList);

		baseStore.setSAPConfiguration(sapConfigModel);
		baseStore.setCatalogs(Arrays.asList(classificationSystemModel));
		...
		// default in hybris is DE/EUR LOG.info("Tests running with locale: " +
		i18NService.getCurrentLocale().toString();
		final CurrencyModel currentCurrency = i18NService.getCurrentCurrency();
		LOG.info("Tests running with Currency: isoCode=" + currentCurrency.getIsocode() + "; sapCode="
				+ currentCurrency.getSapCode());

		LOG.info("CPQ DATA READY FOR TEST!");
	}
