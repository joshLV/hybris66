...
public class CPQHomeTheaterPocConfigMockImpl extends BaseRunTimeConfigMockImpl
{
	...
	private static final String LANG_DEP_NAME_STE = "Stereo";
	...
	protected CsticModel createReceiveModeCstic(final String instanceId)
	{
		...
		builder.stringType().singleSelection();
		builder.addOption(STE, LANG_DEP_NAME_STE).addOption(SUR, LANG_DEP_NAME_SUR);
		builder.withDefaultUIState().readOnly();
		return builder.build();
	}
	...
	protected void addRearSpeakers(final InstanceModel instance)
	{
		...
		if (null == subInstance)
		{
			...
			// cstic groups for 'Rear Speakers' subInstance:
			final List<CsticGroupModel> subInstanceRearSpeakerCsticGroups = new ArrayList<>();
			addCsticGroup(subInstanceRearSpeakerCsticGroups, InstanceModel.GENERAL_GROUP_NAME, "", CPQ_HT_SPK_MODEL,
					CPQ_HT_SPK_COLOR, CPQ_HT_SPK_POWER, CPQ_HT_SPK_INCLUDE_CABLE, CPQ_HT_SPK_INCLUDE_STAND);
			subInstanceRearSpeaker.setCsticGroups(subInstanceRearSpeakerCsticGroups);

			// cstics and values for 'Rear Speakers' subInstance:
			final List<CsticModel> subInstanceRearSpeakerCstics = new ArrayList<>();
			subInstanceRearSpeakerCstics.add(createSpeakerModelCstic(REAR_SPEAKERS_SUBINSTANCE_ID));
			subInstanceRearSpeakerCstics.add(createSpeakerColorCstic(REAR_SPEAKERS_SUBINSTANCE_ID));
			subInstanceRearSpeakerCstics.add(createSpeakerPowerCstic(REAR_SPEAKERS_SUBINSTANCE_ID));
			subInstanceRearSpeakerCstics.add(createSpeakerIncludeCableCstic(REAR_SPEAKERS_SUBINSTANCE_ID));
			subInstanceRearSpeakerCstics.add(createSpeakerIncludeStandCstic(REAR_SPEAKERS_SUBINSTANCE_ID));
			subInstanceRearSpeaker.setCstics(subInstanceRearSpeakerCstics);
		}
	}
	...
	protected void addFrontSpeakers(final InstanceModel instance)
	{
		...
		if (null == subInstance)
		{
			...
			subInstanceFrontSpeakerCstics.add(createSpeakerModelCstic(FRONT_SPEAKERS_SUBINSTANCE_ID));
			subInstanceFrontSpeakerCstics.add(createSpeakerColorCstic(FRONT_SPEAKERS_SUBINSTANCE_ID));
			subInstanceFrontSpeakerCstics.add(createSpeakerPowerCstic(FRONT_SPEAKERS_SUBINSTANCE_ID));
			subInstanceFrontSpeakerCstics.add(createSpeakerIncludeCableCstic(FRONT_SPEAKERS_SUBINSTANCE_ID));
			subInstanceFrontSpeakerCstics.add(createSpeakerIncludeStandCstic(FRONT_SPEAKERS_SUBINSTANCE_ID));
