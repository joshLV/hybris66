...
public class CharonFacadeImpl implements CharonFacade
{
	...
	public CPSConfiguration createDefaultConfiguration(final KBKey kbKey)
	{
		try
		{
			...
			final CPSConfiguration cpsConfig = retrieveConfigurationAndSaveCookies(rawResponse);
			timer.stop();
			return cpsConfig;
		}
	...
	public void releaseSession(final String configId)
	{
		...
		try
		{
			...
			getClient().deleteConfiguration(configId, cookiesAsString.get(0), cookiesAsString.get(1)).subscribeOn(getScheduler())
					.toBlocking().first();
			timer.stop();
		}
		catch (final HttpException ex)
