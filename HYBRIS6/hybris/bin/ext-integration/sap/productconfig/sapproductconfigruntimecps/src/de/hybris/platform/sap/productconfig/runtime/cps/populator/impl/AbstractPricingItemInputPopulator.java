...
public class AbstractPricingItemInputPopulator
{
	...
	protected CPSQuantity createQty(final Double value, final String uom)
	{
		final CPSQuantity qty = new CPSQuantity();
		qty.setUnit(uom);
		qty.setValue(value);
		return qty;
	}

	protected void fillCoreAttributes(final String id, final CPSQuantity quantity, final PricingItemInput target)
	{
		target.setItemId(id);
