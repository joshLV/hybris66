...
public class AbstractPricingItemInputPopulatorTest
{
	...
	{
		classUnderTest.fillCoreAttributes(ITEM_ID, classUnderTest.createQty(Double.valueOf(2), UOM), target);
		assertEquals(ITEM_ID, target.getItemId());
		assertEquals(Double.valueOf(2), target.getQuantity().getValue());
	...
	{
		final CPSQuantity qtyResult = classUnderTest.createQty(Double.valueOf(1), UOM_ST);
		assertNotNull(qtyResult);
		assertEquals(Double.valueOf(1), qtyResult.getValue());
