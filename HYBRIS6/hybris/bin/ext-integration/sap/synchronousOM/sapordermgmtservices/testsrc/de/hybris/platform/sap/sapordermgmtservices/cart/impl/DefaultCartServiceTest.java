...
public class DefaultCartServiceTest
{
	...
	public void init()
	{
		...
		EasyMock
				.expect(productConfigurationService.calculateCartEntryConfigurationAttributes(EasyMock.anyObject(String.class),
						EasyMock.anyObject(String.class), EasyMock.anyObject(String.class)))//
				.andReturn(configAttributes).anyTimes();

		EasyMock.expect(productConfigurationService.retrieveConfigurationModel(null)).andReturn(configModel);
	...
	private Converter<Item, OrderEntryData> createCartItemConverter()
	{
		final Converter<Item, OrderEntryData> converter = new AbstractConverter<Item, OrderEntryData>()
		{
			...
			public void populate(final Item source, final OrderEntryData target)
			{
				...
				final Converter<ConfigModel, List<ConfigurationInfoData>> orderEntryConfigurationInfoConverter = EasyMock
						.createMock(Converter.class);

				populator.setSessionAccessService(sessionAccessService);
				populator.setOrderEntryConfigurationInfoConverter(orderEntryConfigurationInfoConverter);
