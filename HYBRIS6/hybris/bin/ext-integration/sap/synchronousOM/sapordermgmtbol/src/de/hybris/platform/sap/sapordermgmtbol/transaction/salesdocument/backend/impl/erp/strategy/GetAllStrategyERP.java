...
public abstract class GetAllStrategyERP extends BaseStrategyERP implements GetAllStrategy, ConstantsR3Lrd
{
			...
			final ItemBuffer itemBuffer, final JCoConnection cn) throws BackendException
	{
		...
		if (SAP_LOGGER.isDebugEnabled())
		{
			...
			JCoHelper.logCall(function.getName(), function.getImportParameterList().getTable("IT_ITEM_HANDLE"),
					function.getTableParameterList().getTable("TT_ITEM_COMV"), SAP_LOGGER);
		}
		...
		// header tables
		headMapper.read(esHeadComV, esHeadComR, ttHeadCondV, backendState, shop, headerPriceAttribs, itemsPropMap,
