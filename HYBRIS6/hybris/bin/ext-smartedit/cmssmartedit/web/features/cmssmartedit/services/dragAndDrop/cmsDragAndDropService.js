...
(function() {
    ...
    angular.module('cmsDragAndDropServiceModule', ['yjqueryModule', 'dragAndDropServiceModule', 'alertServiceModule', 'yLoDashModule', 'componentHandlerServiceModule', 'eventServiceModule', 'gatewayFactoryModule', 'slotRestrictionsServiceModule', 'componentEditingFacadeModule', 'translationServiceModule', 'removeComponentServiceModule', 'assetsServiceModule', 'browserServiceModule', 'seConstantsModule'])
        ...
        .service('cmsDragAndDropService', function($q, $window, $translate, $timeout, yjQuery, lodash, dragAndDropService, componentHandlerService, systemEventService, gatewayFactory, slotRestrictionsService, alertService, assetsService, browserService, componentEditingFacade, DRAG_AND_DROP_EVENTS, COMPONENT_REMOVED_EVENT, CONTENT_SLOT_TYPE, OVERLAY_RERENDERED_EVENT, SUPPORTED_BROWSERS, CONTRACT_CHANGE_LISTENER_PROCESS_EVENTS, SMARTEDIT_COMPONENT_PROCESS_STATUS, CONTRACT_CHANGE_LISTENER_COMPONENT_PROCESS_STATUS) {
             ...
             * To avoid having the dragged component removed we mark it as "KEEP_VISIBLE" during the whole drag and drop operation.
             * If there is no drag and drop operation in progress, the component status is reset to PROCESS.
             * On drag end, an event is sent to call a RESTART_PROCESS to add or remove the components according to their viewport visibility.
             */
            systemEventService.registerEventHandler(CONTRACT_CHANGE_LISTENER_PROCESS_EVENTS.PROCESS_COMPONENTS, function(evtId, components) {
                ...
                if (this._dragInfo) {
                    result = lodash.map(components, function(component) {
                        var originalComponent = componentHandlerService.getOriginalComponentWithinSlot(this._dragInfo.componentId, this._dragInfo.componentType, this._dragInfo.slotId);
                        if (yjQuery.contains(component, originalComponent.get(0)) || component === originalComponent.get(0)) {
                            component.dataset[SMARTEDIT_COMPONENT_PROCESS_STATUS] = CONTRACT_CHANGE_LISTENER_COMPONENT_PROCESS_STATUS.KEEP_VISIBLE;
                        }
                        return component;
                    }.bind(this));
                } else {
                    result = lodash.map(components, function(component) {
                        component.dataset[SMARTEDIT_COMPONENT_PROCESS_STATUS] = component.dataset[SMARTEDIT_COMPONENT_PROCESS_STATUS] === CONTRACT_CHANGE_LISTENER_COMPONENT_PROCESS_STATUS.REMOVE ? CONTRACT_CHANGE_LISTENER_COMPONENT_PROCESS_STATUS.REMOVE : CONTRACT_CHANGE_LISTENER_COMPONENT_PROCESS_STATUS.PROCESS;
                        return component;
                    });
                }
                    ...
                    if (this._highlightedHint || this._highlightedSlot.components.length === 0) {
                        ...
                        if (!sourceSlotId) {
                            if (!sourceComponentId) {
                                ...
                                result = this._dragInfo.cloneOnDrop ? componentEditingFacade.cloneExistingComponentToSlot(componentProperties) : componentEditingFacade.addExistingComponentToSlot(targetSlotId, this._dragInfo, position);
                            }
                        } else {
            ...
            this._cacheElements = function() {
                ...
                overlay.find(SLOT_SELECTOR).each(function() {
                    ...
                    if (components.length === 0) {
                        ...
                        components.each(function() {
                            var component = yjQuery(this);
                            var componentId = componentHandlerService.getSlotOperationRelatedId(component);
