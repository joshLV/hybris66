        ...
        this.cloneExistingComponentToSlot = function(componentProperties) {
            ...
            return ComponentService.loadComponentItem(componentProperties.dragInfo.componentUuid).then(function(_component) {
                ...
                return sharedDataService.get('experience').then(function(experience) {
                    ...
                    editorModalService.open(componentAttributes, componentProperties.targetSlotId, componentProperties.position).then(function(item) {
                        componentVisibilityAlertService.checkAndAlertOnComponentVisibility({
                            itemId: item.uuid,
                        ...
                        alertService.showSuccess(_generateSuccessMessage(item.uid, componentProperties.targetSlotId));

                        renderService.renderSlots(componentProperties.targetSlotId).then(function() {
                            slotVisibilityService.reloadSlotsInfo();
                        });
            ...
            return _contentSlotComponentsRestService.update({
                ...
                function() {
                    renderService.renderSlots([sourceSlotId, targetSlotId]).then(function() {
                        ...
                        $log.error('componentEditingFacadeModule.moveComponent - unable to call renderSlots - slotIds:', [sourceSlotId, targetSlotId]);
                        return $q.reject(e);
                    });
                },
                function(response) {
