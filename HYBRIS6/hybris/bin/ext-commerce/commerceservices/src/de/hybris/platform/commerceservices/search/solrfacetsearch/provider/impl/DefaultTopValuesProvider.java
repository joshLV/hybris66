...
public class DefaultTopValuesProvider implements FacetTopValuesProvider
{
	...
	public List<FacetValue> getTopValues(final IndexedProperty indexedProperty, final List<FacetValue> facetValues)
	{
		...
		if (facetValues != null)
		{
			final List<FacetValue> sortedFacetValues = new ArrayList<>(facetValues);

			// Sort the facet values by Count in descending order
			Collections.sort(sortedFacetValues, FacetValueCountComparator.INSTANCE);

			for (final FacetValue facetValue : sortedFacetValues)
			{
