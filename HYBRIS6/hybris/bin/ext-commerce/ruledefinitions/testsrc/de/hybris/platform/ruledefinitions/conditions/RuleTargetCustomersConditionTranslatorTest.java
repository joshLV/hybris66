...
public class RuleTargetCustomersConditionTranslatorTest
{
	...
	public void testTranslateNotOperatorCustomerCondition()
	{
		...
		assertThat(irNotCondition.getChildren().get(2), instanceOf(RuleIrAttributeRelCondition.class));
		final RuleIrAttributeRelCondition ruleIrCartConsumedCondition = (RuleIrAttributeRelCondition) irNotCondition.getChildren().get(2);
		assertEquals(RuleIrAttributeOperator.EQUAL, ruleIrCartConsumedCondition.getOperator());
		assertEquals(CART_RAO_VAR, ruleIrCartConsumedCondition.getTargetVariable());

		assertThat(irNotCondition.getChildren().get(3), instanceOf(RuleIrAttributeCondition.class));
		final RuleIrAttributeCondition ruleIrAvailableQtyCondition = (RuleIrAttributeCondition) irNotCondition.getChildren().get(3);
		...
		final RuleIrGroupCondition ruleIrGroupCondition = (RuleIrGroupCondition) irNotCondition.getChildren().get(4);
	...
	public void testTranslateAnyOperatorCondition()
	{
		...
		final RuleIrGroupCondition ruleIrGroupCondition = (RuleIrGroupCondition) irGroupCondition.getChildren().get(4);
		final List<RuleIrCondition> groupConditions = ruleIrGroupCondition.getChildren();
		assertThat(groupConditions.get(0), instanceOf(RuleIrGroupCondition.class));
		final RuleIrGroupCondition irCustomerGroupsCondition = (RuleIrGroupCondition) groupConditions.get(0);
		assertEquals(2, irCustomerGroupsCondition.getChildren().size());
	...
	public void testTranslateAnyOperatorWithExcludedCondition()
	{
		...
		checkExcludeCustomerCondition(irGroupCondition.getChildren().get(5), USER_RAO_ID_ATTRIBUTE);
		final RuleIrGroupCondition ruleIrGroupCondition = (RuleIrGroupCondition) irGroupCondition.getChildren().get(4);
		final List<RuleIrCondition> groupConditions = ruleIrGroupCondition.getChildren();
		assertThat(groupConditions.get(0), instanceOf(RuleIrGroupCondition.class));
		final RuleIrGroupCondition irCustomerGroupsCondition = (RuleIrGroupCondition) groupConditions.get(0);
	...
	public void testTranslateExcludedConditionWithPKAttribute()
	{
		...
		checkExcludeCustomerCondition(irGroupCondition.getChildren().get(5), USER_RAO_PK_ATTRIBUTE);
	}
	...
	protected RuleIrGroupCondition checkFirstLevelChildConditions(final List<RuleIrCondition> ruleIrConditions)
	{
		...
		final RuleIrAttributeRelCondition ruleIrCartConsumedCondition = (RuleIrAttributeRelCondition) ruleIrConditions.get(2);
		assertEquals(RuleIrAttributeOperator.EQUAL, ruleIrCartConsumedCondition.getOperator());
		assertEquals(CART_RAO_VAR, ruleIrCartConsumedCondition.getTargetVariable());

		assertThat(ruleIrConditions.get(3), instanceOf(RuleIrAttributeCondition.class));
		final RuleIrAttributeCondition ruleIrAvailableQtyCondition = (RuleIrAttributeCondition) ruleIrConditions.get(3);
		assertEquals(RuleIrAttributeOperator.GREATER_THAN_OR_EQUAL, ruleIrAvailableQtyCondition.getOperator());
		assertEquals(AVAILABLE_QUANTITY_VAR, ruleIrAvailableQtyCondition.getAttribute());

		assertThat(ruleIrConditions.get(4), instanceOf(RuleIrGroupCondition.class));
		final RuleIrGroupCondition ruleIrGroupCondition = (RuleIrGroupCondition) ruleIrConditions.get(4);
