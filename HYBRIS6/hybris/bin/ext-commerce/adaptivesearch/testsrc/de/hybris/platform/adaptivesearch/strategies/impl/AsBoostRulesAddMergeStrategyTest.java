...
public class AsBoostRulesAddMergeStrategyTest extends AbstractAsBoostRulesMergeStrategyTest
{
	...
	public void mergeBoostRules()
	{
		...
		final Iterator<AsConfigurationHolder<AsBoostRule>> boostRulesIterator = getTarget().getBoostRules().iterator();

		final AsConfigurationHolder<AsBoostRule> boostRule1Holder = boostRulesIterator.next();
		assertSame(boostRule1, boostRule1Holder.getConfiguration());
		assertNull(boostRule1Holder.getReplacedConfigurationUid());

		final AsConfigurationHolder<AsBoostRule> boostRule2Holder = boostRulesIterator.next();
	...
	public void mergeBoostRulesWithDuplicates()
	{
		...
		final AsConfigurationHolder<AsBoostRule> boostRule3Holder = boostRulesIterator.next();
		assertSame(boostRule3, boostRule3Holder.getConfiguration());
		assertNull(boostRule3Holder.getReplacedConfigurationUid());

		final AsConfigurationHolder<AsBoostRule> boostRule4Holder = boostRulesIterator.next();
