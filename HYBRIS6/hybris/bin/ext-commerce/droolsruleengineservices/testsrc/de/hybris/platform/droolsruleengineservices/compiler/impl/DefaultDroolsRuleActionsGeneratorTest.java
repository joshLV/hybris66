...
public class DefaultDroolsRuleActionsGeneratorTest extends AbstractGeneratorTest
{
	...
	@Test
	public void testGenerateCodeForStackbleRule() throws IOException
	{
		// given
		final RuleGroupModel ruleGroup = mock(RuleGroupModel.class);
		when(rule.getRuleGroup()).thenReturn(ruleGroup);
		when(ruleGroup.getCode()).thenReturn("testRuleGroup");
		when(rule.getStackable()).thenReturn(Boolean.TRUE);

		final String expectedDroolsCode = getResourceAsString("/droolsruleengineservices/test/compiler/generatedActionsStackable.bin");

		final BigDecimal totalValue = BigDecimal.valueOf(20);
		final Map<String, Object> actionParameters = Maps.newTreeMap(String::compareTo);
		actionParameters.put("total", totalValue);
		...
		when(droolsRuleValueFormatter.formatValue(droolsRuleGeneratorContext, actionParameters)).thenReturn(
				"[\"stackable\":Boolean.TRUE,\"value\":new BigDecimal(20)]");
		...
		// then
		assertEquals(expectedDroolsCode, generatedDroolsCode);
		assertThat(actionParameters).containsKey("stackable");
	}
}
